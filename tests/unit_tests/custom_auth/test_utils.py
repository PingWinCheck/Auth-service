...
class TestJWT:
    ...
    payload = {"sub": "123", "email": "zxc@asd.qwe"}

    def test_jwt_encode_decode(self):
        encode = self.new_jwt.encode(payload=self.payload, expire_seconds=200)
    ...
    def test_jwt_encode_decode_expire(self):
        encode = self.new_jwt.encode(payload=self.payload, expire_seconds=60)
