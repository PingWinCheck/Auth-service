...
from custom_auth.schemas import TokenSchema
...
class JWT:
    ...
    def encode(self, payload: dict, expire_seconds: int) -> str:
        ...
        return jwt.encode(payload=payload_copy, key=self.key, algorithm=self.algorithm)
    ...
    def decode(self, token: str) -> dict:
        ...
        return jwt.decode(jwt=token, key=key, algorithms=[self.algorithm])
...
class TokenGenerator:
    ...
    def create_access_refresh_tokens_pair(
        self, payload_access: dict, payload_refresh: dict | None = None
    ) -> TokenSchema:
        """
